...
class ComposDF:
    def __init__(self, json_file=None, img_file=None, json_data=None):
        ...
        self.compos_json = self.json_data['compos'] #list of compos
        self.compos_dataframe = self.trans_as_df() # calling trans_as_df
        #print(self.compos_dataframe)
        #self.to_csv("r.csv")
    ...
    def trans_as_df(self):
        ...
        df = pd.DataFrame(columns=list(self.compos_json[0].keys()) + ['area', 'center', 'center_column', 'center_row']) #columns for df
        for i, compo in enumerate(self.compos_json):
            ...
            df.loc[i] = compo
        return df # forms a data frame of compos and features
    ...
    ******************************
    '''
    def repetitive_group_recognition(self, show=False, clean_attrs=True): # call from notebook
        df_nontext = rep.recog_repetition_nontext(self, show) #call this func recog_repetition_nontext
    ...
    def cluster_dbscan_by_attr(self, attr, eps, min_samples=1, show=True, show_method='line'):
        #print(self.compos_dataframe[attr])
        #x = np.reshape(list(self.compos_dataframe[attr]), (-1, 1))
        x= list(self.compos_dataframe[attr])
        x= np.asarray(x)
      
        
        clustering = DBSCAN(eps=eps, min_samples=min_samples).fit(x.reshape(-1,1))
        tag = 'cluster_' + attr
    ...
    ******************************
    ...
    def pair_groups(self):
        ...
        if(len(groups_nontext)==0):
            all_groups = groups_nontext
            
        elif(len(groups_text)==0):
            all_groups = groups_text
        ...
        #print( "after merge with pair",df_all )
        # df_all[list(df_all.filter(like='group'))] = df_all[list(df_all.filter(like='group'))].astype(int)
        if("pair_to" not in df_all.columns):
            df_all["pair_to"]=-1
        if("group_pair" not in df_all.columns):
            df_all["group_pair"]=-1
    ...
    ******************************
    ...
    def list_item_partition(self):
        ...
        #print("hello",listed_compos)

        if(listed_compos.empty):
