...
def recog_repetition_nontext(compos, show=True, inplace=True):
    compos_cp = compos.copy() # object copied in compos cp
    compos_cp.select_by_class(['Compo', 'Background'], replace=True) # select by class of compos object
    #print("compos_cp  non_text dataframe after select by class" , compos_cp.compos_dataframe )
...
def recog_repetition_text(compos, show=True, inplace=True):
    ...
    #print("compos_cp TexT dataframe after select by class" , compos_cp.compos_dataframe )
